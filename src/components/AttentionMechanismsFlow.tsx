...
import { attentionMechanismsPattern } from '../data/patterns/attention-mechanisms';

const nodeTypes = {
  // Custom node types can be added here if needed
};

const AttentionMechanismsFlow: React.FC = () => {
  const [nodes, setNodes, onNodesChange] = useNodesState(attentionMechanismsPattern.initialNodes);
  const [edges, setEdges, onEdgesChange] = useEdgesState(attentionMechanismsPattern.initialEdges);
  ...
  const resetFlow = () => {
    setCurrentStep(0);
    setIsPlaying(false);
    setNodes(attentionMechanismsPattern.initialNodes);
    setEdges(attentionMechanismsPattern.initialEdges);
  };

  const playStep = () => {
    if (currentStep < attentionMechanismsPattern.steps.length) {
      const step = attentionMechanismsPattern.steps[currentStep];
  ...
  const playAll = () => {
    ...
    const interval = setInterval(() => {
      if (stepIndex < attentionMechanismsPattern.steps.length) {
        const step = attentionMechanismsPattern.steps[stepIndex];
  ...
  const currentStepData = currentStep > 0 ? attentionMechanismsPattern.steps[currentStep - 1] : null;
    ...
    <div className="w-full h-screen bg-gray-900">
      ...
      <div className="bg-gray-800 border-b border-gray-700 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-white">{attentionMechanismsPattern.title}</h1>
            <p className="text-gray-400 text-sm mt-1">{attentionMechanismsPattern.description}</p>
          </div>
          <div className="flex gap-2">
            <button
              onClick={playStep}
              disabled={isPlaying || currentStep >= attentionMechanismsPattern.steps.length}
      ...
      <div className="absolute bottom-4 left-4 right-4 bg-gray-800 border border-gray-700 rounded p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-gray-400 text-sm">
            {currentStep} / {attentionMechanismsPattern.steps.length}
          </span>
        </div>
        <div className="w-full bg-gray-700 rounded-full h-2">
          <div
            ...
            style={{
              width: `${(currentStep / attentionMechanismsPattern.steps.length) * 100}%`,
            }}
          />
        </div>
        <div className="mt-2 grid grid-cols-1 md:grid-cols-7 gap-1 text-xs">
          {attentionMechanismsPattern.steps.map((step, index) => (
            <div
              key={step.id}
            ...
            >
              <div className="font-medium text-xs">{step.title.split(' ')[0]}</div>
              <div className="truncate text-xs">{step.title.split(' ').slice(1, 3).join(' ')}</div>
            </div>
          ))}
        </div>
      </div>
...
export default AttentionMechanismsFlow;